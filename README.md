...
| **Tests** | 97 Rust tests on a Mac — a few run only on Unix or only in the build with the bundled embedding model, so the count differs slightly elsewhere — run by CI on Linux, macOS and Windows · 3,662 source checks, every one of them run by CI on every push |
...
```bash
npm run check                                     # 3,662 checks over the real source
