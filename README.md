...
| **Tests** | 122 Rust tests on a Mac — a few run only on Unix or only in the build with the bundled embedding model, so the count differs slightly elsewhere — run by CI on Linux, macOS and Windows · 4,767 source checks, every one of them run by CI on every push |
