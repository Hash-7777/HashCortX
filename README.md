...
| **Stack** | Rust · vanilla JavaScript · no bundler · no framework · ~48,000 lines JS, ~6,100 Rust |
| **Tests** | 135 Rust tests on a Mac — a few run only on Unix or only in the build with the bundled embedding model, so the count differs slightly elsewhere — run by CI on Linux, macOS and Windows · 4,785 source checks, every one of them run by CI on every push |
...
```bash
npm run check                                     # 4,785 checks over the real source
cargo test --manifest-path src-tauri/Cargo.toml   # 135 tests
```
