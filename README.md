...
| **Tests** | 102 Rust tests on a Mac — a few run only on Unix or only in the build with the bundled embedding model, so the count differs slightly elsewhere — run by CI on Linux, macOS and Windows · 4,285 source checks, every one of them run by CI on every push |
...
| 06 | **ERP** | Describe a business, get a working prototype with linked records and stages you can move them through; change it by asking, every version kept |
...
```bash
npm run check                                     # 4,285 checks over the real source
