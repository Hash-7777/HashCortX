...
From the same Workspace you can ask one of the run's agents for a change. That sends the model set for that agent — or, if none is set, the model chosen in the app — the run's task, its conversation and its files, cut to fixed budgets, and nothing else. The agent answers without tools, so it cannot search, fetch or run anything while it does. Its answer changes only the files it names in full, never an unlabelled example, and each change is kept as a new version, so none overwrites what came before. The runs are kept in the app's own IndexedDB store, `hashcortx_swarm_runs`, not in the localStorage that holds your keys.
