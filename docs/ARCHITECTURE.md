...
│   │   ├── app.js            6,542  core: state, chat, agents, tools, providers
...
│   │   ├── code/             1,027  HashCoder: terminal colour, export, file names,
...
- **`npm run bench:coder` measures HashCoder on real tasks.** Twenty small
  ...
  scripted stand-in for a model, to show in seconds that the loop still sends
  the agent back to make a change it wrote into its reply, lands the edit,
  sends it back to prove it, and tells the person what was proven. It needs macOS, a browser and, except for those two, a local model,
  so it is not in CI.
