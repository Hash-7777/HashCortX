...
Roughly **44,500 lines of JavaScript** (plus ~18,700 more in vendored libraries) and **4,900 lines of Rust**. The sizes below were measured on 10 September 2026; the budgets that stop the large files growing are in `scripts/checks/app-size.mjs`, which is the place to look for a current figure.
...
│   │   ├── agent-maker/      2,472  chain / vote / failover
...
│   │   ├── swarm/            1,131  Agent Swarm: layout, scheduling, fencing,
│   │   │                            a site's files, each run kept as a
│   │   │                            conversation with versions (IndexedDB),
│   │   │                            and the Workspace that shows it
...
│   └── tauri.conf.json
│
├── scripts/checks/                  the automated frontend checks — 80 files,
