...
ok('entities the model left empty are asked for, on both ways a spec is made and on a change', (mode.match(/await writeMissingRecords\(await finalizeOrRepairGeneratedSpec\(/g) || []).length === 2 && /if \(made\.newTables\.length\) next = await writeMissingRecords\(next/.test(mode));
...
ok('the model is told what today is, so its dates are not a year out', (mode.match(/Today is \$\{todayIso\(\)\}/g) || []).length === 2 && /Dates are YYYY-MM-DD between \$\{window\.HCSystemsSamples\.daysBefore\(todayIso\(\), 180\)\} and \$\{todayIso\(\)\}/.test(mode));
