...
const ALLOWED = new Map([
  ['platform/index.js', 'defines HC.invoke itself, opens a web link a person clicked in the system browser, plus the usage log, notch and saved-projects bridges. Each takes a value and nothing else — no path, no options — and writes to a destination chosen in Rust, so a mode can use one without being able to aim it'],
