...
Tools: remember_fact / recall_facts — save the user's target roles, industries, and career preferences for more tailored future advice.`,
  ...
  // ── Local model: the walkthrough that checks itself ───────────────────
  //
  // Every claim on this pane is probed rather than assumed. A step only reads
  // "done" because something answered — Ollama's own /api/tags, at the address
  // the user has configured. Nothing here installs, downloads or elevates: the
  // commands are shown for the user to run, and the only button that reaches
  // the network is Test, which pings the loopback port.
  const LM_MODELS = [
