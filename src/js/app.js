...
Tools: remember_fact / recall_facts — save the user's target roles, industries, and career preferences for more tailored future advice.`,
  ...
  async function exportConversation(format) {
    ...
    if (format === "pdf") {
      ...
      // Laid out by js/export-format.js, where it can be checked without a PDF library.
      const doc = window.HCExport.conversationPdf(jsPDF, snapshot, { formatDuration, stripReplyPrelude });
  ...
  // The chip rows live in src/data/prompts.js beside the prompts they send.
  const { COMPOSER_CHIPS } = window.HCPrompts;
  ...
  function renderAgentsList() {
    ...
    kb.querySelector("#ragClearBtn").addEventListener("click", async (e) => {
      ...
      saveRAG([]);
      localStorage.removeItem(window.HCRag.RAG_KEY);
    });
  ...
  function updateDeleteBtnVisibility() {
    ...
    del.hidden = !(selected && selected.dataset.builtin === "0");
  }
  ...
  const settingsPane = $("settingsPane"), localPane = $("localPane"), apisPane = $("apisPane"),
        memoryPane = $("memoryPane"), aboutPane = $("aboutPane");
  ...
  //
  // Fourteen password fields in one column is a wall. The filter matches the
  // label text, so "free" finds every provider whose note says so.
  const apiSearch = $("apiSearch"), apiNoMatch = $("apiNoMatch");
  apiSearch?.addEventListener("input", () => {
    const needle = apiSearch.value.trim().toLowerCase();
    let shown = 0;
    apisPane?.querySelectorAll(".field").forEach((row) => {
    ...
    apisPane?.querySelectorAll("[data-api-group]").forEach((head) => {
      let visible = false;
      for (let el = head.nextElementSibling; el; el = el.nextElementSibling) {
        if (el.hasAttribute("data-api-group")) break;
        if (el.classList.contains("field") && !el.hidden) { visible = true; break; }
      }
      head.hidden = !visible;
    });
    if (apiNoMatch) apiNoMatch.hidden = shown > 0;
  });

  // The Local model walkthrough lives in src/core/settings/local-model.js.
  // It wires its own buttons; the rail only needs to know how to redraw it.
  //
  // Declared as a function, not a const. The settings rail is built inside
  // activateSettingsTab(), which is called at the top of this file long before
  // this line — a const would be in its temporal dead zone there, throw, and
  // take the rest of app.js down with it. It did, until this was written.
  function renderLocalPane() { return window.HCSettingsLocal.renderLocalPane(); }
  ...
  // The Memory pane and the memory map live in src/core/settings/memory-pane.js.
  // The store stays here — the agent layer writes through the same four
  // functions, so the facts keep one source of truth.
  // A function for the same reason as renderLocalPane above: the rail reads
  // this name before this line is reached.
  function renderMemoryPane(...args) { return window.HCSettingsMemory.renderMemoryPane(...args); }
  window.HCSettingsMemory.init({
  ...
  // The fallback catalogue lives in src/data/cloud-models.js. It is a table
  ...
  function setStatus(kind, text) {
    ...
    // The sidebar is narrow and some of these run long ("Local host offline ·
    // cloud ready"). It now ellipses rather than clipping mid-word, but an
    // ellipsis still hides information — the title puts it back on hover.
  ...
  // ========= Rendering =========
  // The prompt library lives in src/data/prompts.js. It is prose, and the only
  // two names the rest of this file needs from it are these.
  const { PRESET_PROMPTS, FORGE_ARCHITECT_PROMPT } = window.HCPrompts;
  ...
  // chat: cloud models go through streamWithModelValue above so they reach
  ...
  // The memory store lives in src/core/memory/store.js. The settings pane and
  // the agent tools both read through it, so the facts keep one home.
  window.HCMemoryStore.init({ state, uid, currentProject, DEFAULT_PROJECT_ID });
  const MEM_KEY = window.HCMemoryStore.MEM_KEY;
  function memLoad(...a)   { return window.HCMemoryStore.memLoad(...a); }
  function memSave(...a)   { return window.HCMemoryStore.memSave(...a); }
  function memAdd(...a)    { return window.HCMemoryStore.memAdd(...a); }
  function memRecall(...a) { return window.HCMemoryStore.memRecall(...a); }
  function memClear(...a)  { return window.HCMemoryStore.memClear(...a); }
  ...
  // toggle changes while the app runs.
  // Function declarations, not consts. Every one of these names is read
  // higher up this file than it is written, and a const would be in its
  // temporal dead zone there — which throws and takes the rest of app.js with
  // it. check:extraction refuses the const form for exactly this reason.
  window.HCRag.init({ isRagEnabled: () => ragEnabled });
  function addToRAG(...a)       { return window.HCRag.addToRAG(...a); }
  function ingestIntoRAG(...a)  { return window.HCRag.ingestIntoRAG(...a); }
  function loadRAG(...a)        { return window.HCRag.loadRAG(...a); }
  function saveRAG(...a)        { return window.HCRag.saveRAG(...a); }
  function queryRAGMerged(...a) { return window.HCRag.queryRAGMerged(...a); }
  function ragBootLoad(...a)    { return window.HCRag.ragBootLoad(...a); }
