...
(function () {
  ...
  /**
   * Lay a conversation snapshot out as a PDF with the jsPDF constructor it is
   * handed, and return the document. Saving it is the caller's: the app writes
   * the bytes itself, because jsPDF's own save never worked in the app.
   * `deps.formatDuration` and `deps.stripReplyPrelude` are the app's own, so
   * the PDF says what the screen says.
   */
  function conversationPdf(jsPDF, snapshot, deps = {}) {
    ...
    for (const m of snapshot.messages) {
      ...
      const bits = [];
      if (m.durationMs) bits.push(deps.formatDuration ? deps.formatDuration(m.durationMs) : fmtDuration(m.durationMs));
      ...
      // Body
      let body = m.role === "user" ? (deps.stripReplyPrelude ? deps.stripReplyPrelude(m.content || "") : (m.content || "")) : (m.content || "");
      ...
      body = P(markdownToPlainText(body));
