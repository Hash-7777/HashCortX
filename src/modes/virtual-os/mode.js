...
const VoidStudio = (() => {
   ...
   *
   * The archive itself is written by src/js/io/zip.js. There used to be a
   * second implementation of the format here — its own header writer, its own
   * checksum — which did the same job as the one the 3D mode already had, and
   * only one of the two was ever checked against a real archive tool.
   *
  ...
  // ── A model's answer, turned into files ─────────────────────────────────
  //
  // Whether anything usable comes out of a generation is decided in
  ...
  const kindLabel = (item) => ANSWER().kindLabel(item);
  ...
  function makeZip(entries) {
    ...
    const bytes = window.HCZip.store((entries || []).map((entry) => {
      const isFolder = String(entry.name || "").endsWith("/");
      const name = normalizeVirtualPath(entry.name) + (isFolder ? "/" : "");
      return { name, bytes: isFolder ? new Uint8Array(0) : encoder.encode(String(entry.data ?? "")) };
    }));
