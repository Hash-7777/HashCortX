...
const FinanceMode = (() => {
  ...
  /* ── persistence ────────────────────────────────────────────────── */
  // ── Reading money, and adding it up ─────────────────────────────────────
  //
  // Everything this mode shows rests on whether a figure in a cell was read
  // correctly and whether it was counted as money in or money out. Both live
  // in src/js/finance/amounts.js, where they can be handed a figure and asked
...
<div class="fin-body">
  ...
  /* ── bar chart ──────────────────────────────────────────────────── */
  /**
   * The charts, in src/js/finance/charts.js.
   *
   * A chart is read faster than the table beside it and trusted more, so they
   * live where they can be handed figures and asked what they drew.
   */
  const CHARTS = () => window.HCFinanceCharts;
  const renderBarChart   = (c, svgId) => CHARTS().renderBarChart(c, svgId);
  const renderLineChart  = (c, svgId) => CHARTS().renderLineChart(c, svgId);
  const renderDonutChart = (c, svgId) => CHARTS().renderDonutChart(c, svgId);
